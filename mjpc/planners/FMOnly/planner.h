...
#ifndef MJPC_PLANNERS_FMONLY_PLANNER_H_
#define MJPC_PLANNERS_FMONLY_PLANNER_H_
...
inline constexpr int MinSamplingSplinePointsFMOnly = 1;
inline constexpr int MaxSamplingSplinePointsFMOnly = 64;
inline constexpr double MinNoiseStdDevFMOnly = 0.0;
inline constexpr double MaxNoiseStdDevFMOnly = 1.0;

class FMOnlyPlanner : public RankedPlanner {
 public:
  // constructor
  FMOnlyPlanner() = default;

  // destructor
  ~FMOnlyPlanner() override;
  ...
  // policy
  FMOnlyPolicy policy;  // (Guarded by mtx_)
  FMOnlyPolicy candidate_policy[kMaxTrajectory];
  FMOnlyPolicy previous_policy;
...
mjpc::spline::SplineInterpolation interpolation_ =
  ...
  mjData* act_data_ = nullptr;        // dedicated scratch for ActionFromPolicy
  mutable std::mutex act_data_mutex_;  // protects act_data_ (1kHz sim thread)
...
#endif  // MJPC_PLANNERS_FMONLY_PLANNER_H_
