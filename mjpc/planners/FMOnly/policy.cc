...
// allocate memory
void FMOnlyPolicy::Allocate(const mjModel* model, const Task& task,
                              int horizon) {
  // model
  this->model = model;
...
void FMOnlyPolicy::Reset(int horizon, const double* initial_repeated_action) {
  plan.Clear();
...
void FMOnlyPolicy::Action(double* action, const double* state,
                            double time) const {
...
// copy policy
void FMOnlyPolicy::CopyFrom(const FMOnlyPolicy& policy, int horizon) {
  this->plan = policy.plan;
  num_spline_points = policy.num_spline_points;
}

// copy parameters
void FMOnlyPolicy::SetPlan(const TimeSpline& plan) {
  this->plan = plan;
}
