...
// allocate memory
void FlowMPPIPolicy::Allocate(const mjModel* model, const Task& task,
                              int horizon) {
  // model
  this->model = model;
...
void FlowMPPIPolicy::Reset(int horizon, const double* initial_repeated_action) {
  plan.Clear();
...
void FlowMPPIPolicy::Action(double* action, const double* state,
                            double time) const {
...
// copy policy
void FlowMPPIPolicy::CopyFrom(const FlowMPPIPolicy& policy, int horizon) {
  this->plan = policy.plan;
  num_spline_points = policy.num_spline_points;
}

// copy parameters
void FlowMPPIPolicy::SetPlan(const TimeSpline& plan) {
  this->plan = plan;
}
