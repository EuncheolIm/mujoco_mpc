...
FlowMPPIRpyPlanner::~FlowMPPIRpyPlanner() {
  if (ws_data_) {
...
void FlowMPPIRpyPlanner::Initialize(mjModel* model, const Task& task) {
  ...
  // Adaptive sampling sigma - OPT-IN, see planner.h. Absent numeric => inert.
  sigma_adapt_ = GetNumberOrDefault(0.0, model, "sampling_sigma_adapt");
  if (sigma_adapt_ > 0.0) {
    sigma_adapt_min_   = GetNumberOrDefault(0.05, model, "sampling_sigma_adapt_min");
    sigma_adapt_decay_ = GetNumberOrDefault(0.97, model, "sampling_sigma_adapt_decay");
    sigma_adapt_grow_  = GetNumberOrDefault(1000.0, model, "sampling_sigma_adapt_grow");
    sigma_adapt_thr_   = GetNumberOrDefault(0.010, model, "sampling_sigma_adapt_thr");
    sigma_adapt_thr_ori_ = GetNumberOrDefault(0.0, model, "sampling_sigma_adapt_thr_ori");
    sigma_adapt_hyst_  = GetNumberOrDefault(3.0, model, "sampling_sigma_adapt_hyst");
    ...
    if (const char* e = std::getenv("MJPC_SIGMA_ADAPT"); e && e[0])
      sigma_adapt_ = std::atof(e);
    std::fprintf(stderr,
                 "[FlowMPPIRpy] adaptive sigma %s: min=%.3g decay=%.3g thr=%.4g "
                 "thr_ori=%.4g res=[%d,%d)\n",
                 sigma_adapt_ > 0.0 ? "ON" : "OFF", sigma_adapt_min_,
                 sigma_adapt_decay_, sigma_adapt_thr_, sigma_adapt_thr_ori_,
                 sigma_adapt_res_off_, sigma_adapt_res_off_ + sigma_adapt_res_dim_);
  }
  ...
  if (task.Name() != "G1 Stand") {
    if (const char* e = std::getenv("MJPC_TRAJECTORIES"); e && e[0]) {
      ...
      if (v > 0) {
        num_trajectory_ = v;
        std::fprintf(stderr,
                     "[FlowMPPIRpy] MJPC_TRAJECTORIES override: N=%d\n", v);
      }
    }
  }
  ...
  if (const char* e = std::getenv("MJPC_LAMBDA"); e && e[0]) {
    ...
    if (v > 0) {
      mppi_lambda_ = v;
      std::fprintf(stderr, "[FlowMPPIRpy] MJPC_LAMBDA override: lambda=%g\n", v);
    }
  }
...
// allocate memory
void FlowMPPIRpyPlanner::Allocate() {
  // initial state
...
// set state
void FlowMPPIRpyPlanner::SetState(const State& state) {
  state.CopyTo(this->state.data(), this->mocap.data(), this->userdata.data(),
               &this->time);
}

int FlowMPPIRpyPlanner::OptimizePolicyCandidates(int ncandidates, int horizon,
                                              ThreadPool& pool) {
  ...
  // Gate on the absolute TASK ERROR from the best rollout's first step, NOT on
  // cost: cost-relative tests either reset every replan (converged cost still
  // fluctuates ~2x) or silently track a degradation until the controller is
  // starved. Shrink below thr, restore above thr*hyst, hold in between.
  if (sigma_adapt_ > 0.0) {
    int best_i = 0;
    ...
    double err = 0.0;
    const int rdim = sigma_adapt_res_dim_;
    if (rdim > 0 && task->num_residual >= sigma_adapt_res_off_ + rdim) {
      const double* r = trajectory[best_i].residual.data() + sigma_adapt_res_off_;
      const int npos = std::min(3, rdim);
      double pos_n = 0.0;
      for (int k = 0; k < npos; ++k) pos_n += r[k] * r[k];
      err = std::sqrt(pos_n) / std::max(sigma_adapt_thr_, 1e-12);
      if (rdim > 3 && sigma_adapt_thr_ori_ > 0.0) {
        double ori_n = 0.0;
        for (int k = 3; k < rdim; ++k) ori_n += r[k] * r[k];
        err = std::max(err, std::sqrt(ori_n) / sigma_adapt_thr_ori_);
      }
    }
    if (err > sigma_adapt_hyst_) {
      sigma_adapt_scale_ =
          std::min(1.0, sigma_adapt_scale_ * std::max(sigma_adapt_grow_, 1.01));
    } else if (err < 1.0) {
      sigma_adapt_scale_ =
          std::max(sigma_adapt_min_, sigma_adapt_scale_ * sigma_adapt_decay_);
    }
    sigma_adapt_err_ = err;
    sigma_adapt_init_ = true;
    ...
    if (adapt_log > 0) {
      ...
      if (it % adapt_log == 0)
        std::fprintf(stderr,
                     "[sigma_adapt] it=%ld scale=%.4f err=%.5f best=%.6g\n", it,
                     sigma_adapt_scale_, err, best);
    }
  }
  ...
  if (!printed_frac_) {
    std::fprintf(stderr,
                 "[FlowMPPIRpy] fm_frac=%.3f  N_fm=%d  N_mppi=%d\n",
  ...
  static const bool minmax_norm = [] {
    ...
    if (on) std::fprintf(stderr, "[FlowMPPIRpy] MJPC_MINMAX_NORM=1: min-max cost normalization ON\n");
    return on;
  }();
  ...
  // ---- Per-joint cost separation (opt-in: MJPC_PERJOINT=1) -------------------
  // Instead of ONE softmax over the whole cost, give each arm joint its OWN
  // softmax whose cost = shared EE/collision cost + that joint's own
  // centering/velocity/limit reg. A joint near its limit is then steered by its
  // own reg without being diluted by (nor diluting) the other joints. Active
  // only when FM is off (N_fm==0). Requires a task numeric `perjoint_ctrl`
  // listing the control (actuator) index for each arm joint, in joint_cent order.
  static const bool perjoint_env =
      [] { const char* e = std::getenv("MJPC_PERJOINT"); return e && e[0] == '1'; }();
  struct PJInfo { bool ok=false; int naj=0, nterm=0;
                  int cent_ti=-1, vel_ti=-1, lim_ti=-1,
                      cent_off=0, vel_off=0, lim_off=0;
                  std::vector<int> ctrl; };
  static const PJInfo pj = [&]() -> PJInfo {
    PJInfo p; if (!perjoint_env) return p;
    int ti = 0, off = 0;
    for (int s = 0; s < model->nsensor; ++s) {
      if (model->sensor_type[s] != mjSENS_USER) continue;
      const char* sn = mj_id2name(model, mjOBJ_SENSOR, s);
      int dim = model->sensor_dim[s];
      std::string nm = sn ? sn : "";
      if (nm == "joint_cent")      { p.cent_ti = ti; p.cent_off = off; p.naj = dim; }
      else if (nm == "joint_vel" ||
               nm == "joint_vel_penalty") { p.vel_ti = ti; p.vel_off = off; }
      else if (nm == "joint_limit"){ p.lim_ti = ti; p.lim_off = off; }
      off += dim; ++ti;
    }
    p.nterm = ti;
    int nid = mj_name2id(model, mjOBJ_NUMERIC, "perjoint_ctrl");
    if (p.cent_ti >= 0 && p.naj > 0 && nid >= 0 &&
        model->numeric_size[nid] == p.naj) {
      int adr = model->numeric_adr[nid];
      p.ctrl.resize(p.naj);
      for (int d = 0; d < p.naj; ++d) p.ctrl[d] = (int)model->numeric_data[adr + d];
      p.ok = true;
      std::fprintf(stderr, "[PERJOINT] ON: %d arm joints, per-joint softmax\n", p.naj);
    } else {
      std::fprintf(stderr, "[PERJOINT] disabled: need `joint_cent` sensor + "
                           "`perjoint_ctrl` numeric of size naj (got naj=%d nid=%d)\n",
                   p.naj, nid);
    }
    return p;
  }();
  const bool perjoint_use = pj.ok && (N_fm == 0);

  std::vector<std::vector<double>> wj;   // wj[joint][rollout]
  if (perjoint_use) {
    const int naj = pj.naj;
    std::vector<double> shared(num_trajectory, 0.0);
    std::vector<std::vector<double>> reg(naj, std::vector<double>(num_trajectory, 0.0));
    double terms[kMaxCostTerms];
    for (int i = 0; i < num_trajectory; ++i) {
      const int H = trajectory[i].horizon;
      for (int t = 0; t < H; ++t) {
        const double* r = trajectory[i].residual.data() + t * task->num_residual;
        task->CostTerms(terms, r);
        for (int k = 0; k < pj.nterm; ++k)
          if (k != pj.cent_ti && k != pj.vel_ti && k != pj.lim_ti)
            shared[i] += terms[k];                       // EE + collision: shared
        const int offs[3] = {pj.cent_off, pj.vel_off, pj.lim_off};
        const int tis[3]  = {pj.cent_ti,  pj.vel_ti,  pj.lim_ti};
        for (int e = 0; e < 3; ++e) {                    // split reg term by r^2 share
          if (tis[e] < 0) continue;
          double ss = 0.0;
          for (int d = 0; d < naj; ++d) { double v = r[offs[e]+d]; ss += v*v; }
          if (ss > 1e-12)
            for (int d = 0; d < naj; ++d) { double v = r[offs[e]+d]; reg[d][i] += terms[tis[e]]*(v*v/ss); }
          else
            for (int d = 0; d < naj; ++d) reg[d][i] += terms[tis[e]]/naj;
        }
      }
    }
    wj.assign(naj, std::vector<double>(num_trajectory, 0.0));
    for (int d = 0; d < naj; ++d) {                      // per-joint softmax
      double cmin = 1e300, cmax = -1e300;
      std::vector<double> c(num_trajectory);
      for (int i = 0; i < num_trajectory; ++i) {
        c[i] = shared[i] + reg[d][i];
        cmin = std::min(cmin, c[i]); cmax = std::max(cmax, c[i]);
      }
      double den = (minmax_norm && (cmax - cmin) > 1e-9)
                       ? (cmax - cmin) * mppi_lambda_ : mppi_lambda_;
      double sum = 0.0;
      for (int i = 0; i < num_trajectory; ++i) { wj[d][i] = std::exp(-(c[i]-cmin)/den); sum += wj[d][i]; }
      if (sum > 0) for (int i = 0; i < num_trajectory; ++i) wj[d][i] /= sum;
    }
  }
  ...
  {  // <-- 잠금 시작
    ...
    if (perjoint_use) {
      // Per-joint weighted average around mppi_nominal_. Each arm control dim
      // is averaged with ITS OWN joint weights; any other dim (e.g. grippers)
      // falls back to the shared `weights`. Sum_i w = 1 per column, so the
      // result is the per-dim weighted mean of the candidates.
      std::vector<const double*> wcol(model->nu, weights.data());
      for (int d = 0; d < pj.naj; ++d)
        if (pj.ctrl[d] >= 0 && pj.ctrl[d] < model->nu) wcol[pj.ctrl[d]] = wj[d].data();
      ...
      for (int i = 0; i < num_trajectory; ++i) {
        for (int t = 0; t < mppi_nominal_.plan.Size(); ++t) {
          ...
          for (int k = 0; k < model->nu; ++k)
            base_node->values()[k] +=
                wcol[k][i] * (cand_node->values()[k] - pol_node->values()[k]);
        }
      }
  ...
  // ----- Per-step diagnostic CSV (enabled via MJPC_FLOWMPPIRPY_LOG=path.csv) -----
  ...
  //                       smooth accumulation; large jumps = no accumulation.
  {
    ...
    if (!log_inited) {
      log_inited = true;
      const char* p = std::getenv("MJPC_FLOWMPPIRPY_LOG");
      if (p && p[0]) {
        ...
        if (log_ofs.is_open()) {
          ...
          log_enabled = true;
          std::fprintf(stderr, "[FlowMPPIRpy] diag log -> %s\n", p);
        }
      }
    }
    if (log_enabled) {
...
void FlowMPPIRpyPlanner::OptimizePolicy(int horizon, ThreadPool& pool) {
...
void FlowMPPIRpyPlanner::NominalTrajectory(int horizon, ThreadPool& pool) {
  // set policy
...
void FlowMPPIRpyPlanner::ActionFromPolicy(double* action, const double* state,
...
// nominal.
void FlowMPPIRpyPlanner::UpdateNominalPolicy(int horizon) {
  // dimensions
...
void FlowMPPIRpyPlanner::AddNoiseToPolicy(double start_time, int i, double scale) {
  // start timer
...
void FlowMPPIRpyPlanner::Rollouts(int num_trajectory, int horizon,
                               ThreadPool& pool) {
  ...
  for (int i = 0; i < num_trajectory; i++) {
    pool.Schedule([&s = *this, &model = this->model, &task = this->task,
      ...
      if (no_anchor || (i != 0 && i != N_fm)) {
        ...
        // Adaptive sigma multiplier; pinned at 1.0 unless the task opted in.
        s.AddNoiseToPolicy(time, i, scale * s.sigma_adapt_scale_);
      }
...
const Trajectory* FlowMPPIRpyPlanner::BestTrajectory() {
  return winner >= 0 ? &trajectory[winner] : nullptr;
}

// visualize planner-specific traces
void FlowMPPIRpyPlanner::Traces(mjvScene* scn) {
  // sample color
  float color[4];
  color[0] = 1.0;
  color[1] = 1.0;
...
void FlowMPPIRpyPlanner::GUI(mjUI& ui) {
  mjuiDef defSampling[] = {
  ...
  mju::sprintf_arr(defSampling[2].other, "%i %i", MinSamplingSplinePointsFlowRpy,
                   MaxSamplingSplinePointsFlowRpy);

  // set noise standard deviation limits
  mju::sprintf_arr(defSampling[3].other, "%f %f", MinNoiseStdDevFlowRpy,
                   MaxNoiseStdDevFlowRpy);
...
void FlowMPPIRpyPlanner::Plots(mjvFigure* fig_planner, mjvFigure* fig_timer,
...
double FlowMPPIRpyPlanner::CandidateScore(int candidate) const {
  return trajectory[trajectory_order[candidate]].total_return;
}

// set action from candidate policy
void FlowMPPIRpyPlanner::ActionFromCandidatePolicy(double* action, int candidate,
                                                const double* state,
                                                double time) {
  candidate_policy[trajectory_order[candidate]].Action(action, state, time);
}

void FlowMPPIRpyPlanner::CopyCandidateToPolicy(int candidate) {
  // set winner
...
void FlowMPPIRpyPlanner::UpdateFM() {
  const FMConfig& fmc = GetFMConfig();
  ...
  if (use_clik) {
    if (!clik_tried_) {
      ...
      try {
        ...
        clik_loaded_ = true;
        std::printf("[FlowMPPIRpy] CLIK guide loaded: kp_pos=%.1f kp_ori=%.1f "
                    "damp=%.3f dt=%.3f H=%d\n",
                    fmc.clik_kp_pos, fmc.clik_kp_ori, fmc.clik_damp,
                    fmc.fm_chunk_dt, fmc.clik_horizon);
      } catch (const std::exception& e) {
        std::fprintf(stderr,
            "[FlowMPPIRpy] CLIK guide exception: %s — guide disabled.\n",
            e.what());
        clik_policy_.reset();
      }
    }
    ...
    if (!mlp_tried_) {
      ...
      if (ckpt.empty() || stats.empty()) {
        std::fprintf(stderr,
            "[FlowMPPIRpy] guide_type=mlp but mlp_checkpoint / mlp_stats "
        ...
        try {
          ...
          if (mlp_policy_->isLoaded()) {
            mlp_loaded_ = true;
            std::printf("[FlowMPPIRpy] MLP guide loaded: state=%d action=%d "
                        "horizon=%d\n",
            ...
            std::fprintf(stderr,
                "[FlowMPPIRpy] MLP guide load FAILED — guide disabled.\n");
            mlp_policy_.reset();
          }
        } catch (const std::exception& e) {
          std::fprintf(stderr,
              "[FlowMPPIRpy] MLP guide exception: %s — guide disabled.\n",
              e.what());
          mlp_policy_.reset();
        }
      }
      ...
      if (!ckpt.empty() && !stats.empty()) {
        try {
          ...
          if (fm_policy_->isLoaded()) {
            ...
            fm_loaded_ = true;
            std::printf("[FlowMPPIRpy] FM loaded: state=%d action=%d horizon=%d "
                        "ode_steps=%d\n",
          ...
          std::printf("[FlowMPPIRpy] FM load failed: %s\n", e.what());
          fm_policy_.reset();
        }
      }
    }
  ...
  // Goal: pos + RPY (6-D) from target_site (mocap-driven), matching the adaLN
  // H-gripper FM training convention (goal = [x,y,z,roll,pitch,yaw]).
  // RPY are ZYX-intrinsic Euler from the target rotation (== scipy
  // Rotation.as_euler('ZYX') -> (yaw,pitch,roll)). CRITICAL: the training
  // distribution only saw roll ~ -pi (gripper-down); scipy/atan2 can return
  // roll ~ +pi for the same pose, which after normalization is a ~20-sigma
  // outlier and makes the flow diverge. So fold +pi->-pi and clamp roll/pitch/
  // yaw to the training range (interactive_follow.py::quat_to_goal_rpy).
  Eigen::VectorXd goal = Eigen::VectorXd::Zero(6);
  if (target_site_id_ >= 0) {
    for (int i = 0; i < 3; ++i) {
      goal(i) = ws_data_->site_xpos[3 * target_site_id_ + i];
    }
    const double* R = ws_data_->site_xmat + 9 * target_site_id_;  // row-major
    // ZYX-intrinsic Euler from rotation matrix.
    double yaw   = std::atan2(R[3], R[0]);              // atan2(R10, R00)
    double s = -R[6]; s = std::max(-1.0, std::min(1.0, s));
    double pitch = std::asin(s);                        // asin(-R20)
    double roll  = std::atan2(R[7], R[8]);              // atan2(R21, R22)
    if (roll > 0.0) roll -= 2.0 * M_PI;                 // (1) fold +pi -> -pi
    roll  = std::max(-M_PI - 0.5, std::min(-M_PI + 0.5, roll));   // (2) roll clamp
    pitch = std::max(-1.0, std::min(1.0, pitch));                 // (3) pitch clamp
    yaw   = std::max(-1.5, std::min(1.5, yaw));                   //     yaw clamp
    goal(3) = roll; goal(4) = pitch; goal(5) = yaw;
  }
...
void FlowMPPIRpyPlanner::PublishFMTarget() {
...
void FlowMPPIRpyPlanner::ApplyWarmstart() {
  ...
  for (int t = 0; t < num_knots; ++t) {
    ...
    if (!mask_initialized) {
      ...
      if (const char* e = std::getenv("MJPC_FM_JOINT_MASK"); e && e[0]) {
        ...
        std::fprintf(stderr,
            "[FlowMPPIRpy] FM joint mask: [%.2f %.2f %.2f %.2f %.2f %.2f %.2f]\n",
