...
// allocate memory
void FlowMPPIRpyPolicy::Allocate(const mjModel* model, const Task& task,
                              int horizon) {
  // model
  this->model = model;
...
void FlowMPPIRpyPolicy::Reset(int horizon, const double* initial_repeated_action) {
  plan.Clear();
...
void FlowMPPIRpyPolicy::Action(double* action, const double* state,
                            double time) const {
...
// copy policy
void FlowMPPIRpyPolicy::CopyFrom(const FlowMPPIRpyPolicy& policy, int horizon) {
  this->plan = policy.plan;
  num_spline_points = policy.num_spline_points;
}

// copy parameters
void FlowMPPIRpyPolicy::SetPlan(const TimeSpline& plan) {
  this->plan = plan;
}
