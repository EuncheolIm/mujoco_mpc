...
// allocate memory
void MPOPIPolicy::Allocate(const mjModel* model, const Task& task,
                              int horizon) {
  // model
  this->model = model;
...
void MPOPIPolicy::Reset(int horizon, const double* initial_repeated_action) {
  plan.Clear();
...
void MPOPIPolicy::Action(double* action, const double* state,
                            double time) const {
...
// copy policy
void MPOPIPolicy::CopyFrom(const MPOPIPolicy& policy, int horizon) {
  this->plan = policy.plan;
  num_spline_points = policy.num_spline_points;
}

// copy parameters
void MPOPIPolicy::SetPlan(const TimeSpline& plan) {
  this->plan = plan;
}
