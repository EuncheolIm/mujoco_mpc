...
std::string FR3HGripperCoCarry::XmlPath() const {
  // 질량 축(난이도 2축)은 **XML 변형 파일**로 고른다. MuJoCo 는 body mass 를 바꾸면
  // 관성/합성질량 파생값을 다시 계산해야 하고(mj_setConst), 플래너가 들고 있는 모델
  // 사본에는 그 변경이 전파되지 않는다 -- 런타임 질량 변경은 롤아웃과 실제 물리가
  // 서로 다른 냄비를 들게 만든다. 파일을 갈아 끼우면 두 모델이 항상 일치한다.
  //   MJPC_CC_MASS 없음/1 -> task.xml (1 kg, 기준선)
  //   MJPC_CC_MASS=2      -> task_m2.xml
  //   MJPC_CC_MASS=3      -> task_m3.xml
  // (원본 PotDual 의 MJPC_DUAL_PRIM 분기는 참조하던 task_prim.xml 이 그 디렉터리에
  //  존재하지 않는 죽은 코드였으므로 옮기지 않았다.)
  if (const char* e = std::getenv("MJPC_CC_MASS"); e && e[0]) {
    const int m = std::atoi(e);
    if (m == 2) return GetModelPath("Fr3HGripperCoCarry/task_m2.xml");
    if (m == 3) return GetModelPath("Fr3HGripperCoCarry/task_m3.xml");
  }
  return GetModelPath("Fr3HGripperCoCarry/task.xml");
}
std::string FR3HGripperCoCarry::Name() const { return "FR3_H_Gripper_CoCarry"; }

void FR3HGripperCoCarry::ResidualFn::Residual(const mjModel* model,
                                           const mjData* data,
...
void FR3HGripperCoCarry::TransitionLocked(mjModel* model, mjData* data) {
