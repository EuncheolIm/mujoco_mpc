...
int CostObstacle(const mjModel* model, const mjData* data, double* residual) {
  ...
  // (obj_a/obj_b — the obstacle segment — are computed below, after env_or_num.)
  ...
  const int b6 = mj_name2id(model, mjOBJ_BODY, "fr3_link6");  // fills link5->link7 gap
  ...
  if (b3 < 0 || b4 < 0 || b5 < 0 || b6 < 0 || b7 < 0 || se7 < 0 || tip < 0) {
    residual[0] = 0.0;
    return 1;
  }
  ...
  const double* link6  = data->xpos      + 3 * b6;
  const double* link7  = data->xpos      + 3 * b7;
  const double* linke7 = data->site_xpos + 3 * se7;
  const double* tip_p  = data->site_xpos + 3 * tip;

  // Env (fm_config.yaml -> MJPC_OBS_*) overrides the task.xml numeric, which
  // overrides the hardcoded default. Lets thresholds be tuned from the yaml.
  auto env_or_num = [&](const char* env, const char* num, double def) {
    if (const char* e = std::getenv(env); e && e[0]) return std::atof(e);
    return GetNumberOrDefault(def, model, num);
  };
  const double tip_thr = env_or_num("MJPC_OBS_TIP_THR", "obstacle_tip_thr", 0.12);
  // Extra safety margin added to EVERY arm-vs-obstacle keep-out (widens the
  // hand/arm clearance to the obstacle). yaml: obstacle_extra_margin.
  const double em = env_or_num("MJPC_OBS_EXTRA_MARGIN", "obstacle_extra_margin", 0.0);

  // Obstacle as a segment along y (the cylinder axis): obj_a..obj_b span
  // +-obstacle_half_y about the body center. half_y=0 => degenerate point
  // (sphere), preserving the old behavior. The y-cylinder wall uses the segment.
  const double half_y = env_or_num("MJPC_OBS_HALF_Y", "obstacle_half_y", 0.0);
  const double obj_a[3] = {o[0], o[1] - half_y, o[2]};
  const double obj_b[3] = {o[0], o[1] + half_y, o[2]};

  // CONTINUOUS hinge (MJPC_OBS_CONT=1): residual = Σ max(0, thr - d) over all
  // pairs (penetration depth, meters). Unlike the sparse integer count this has
  // a smooth repulsive GRADIENT, so MPPI knows which way to dodge — essential
  // for a MOVING obstacle (the sparse cost only worked for a static one). The
  // Obstacle sensor weight must be large (~1e6) since penetration is ~0.0–0.2 m.
  static const bool cont = std::getenv("MJPC_OBS_CONT") != nullptr;
  if (cont) {
    auto hinge = [](double d2, double thr) {
      double p = thr - std::sqrt(d2);
      return p > 0.0 ? p : 0.0;
    };
    double pen = 0.0;
    pen += hinge(SegmentDistSq(link0, link2, link4, link5),  0.22);
    pen += hinge(SegmentDistSq(link0, link2, link7, linke7), 0.22);
    pen += hinge(SegmentDistSq(link2, link3, link7, linke7), 0.23);
    pen += hinge(SegmentDistSq(link2, link3, obj_a, obj_b),  0.20 + em);
    pen += hinge(SegmentDistSq(link4, link5, obj_a, obj_b),  0.20 + em);
    pen += hinge(SegmentDistSq(link5, link6, obj_a, obj_b),  0.20 + em);  // gap: forearm->wrist
    pen += hinge(SegmentDistSq(link6, link7, obj_a, obj_b),  0.18 + em);  // gap: link6->wrist
    pen += hinge(SegmentDistSq(link7, linke7, obj_a, obj_b), 0.18 + em);
    pen += hinge(SegmentDistSq(tip_p, tip_p, obj_a, obj_b),  tip_thr + em);
    residual[0] = pen;
    return 1;
  }
  ...
  if (std::sqrt(SegmentDistSq(link2, link3, obj_a, obj_b))  < 0.20 + em) n += 1.0;
  if (std::sqrt(SegmentDistSq(link4, link5, obj_a, obj_b))  < 0.20 + em) n += 1.0;
  if (std::sqrt(SegmentDistSq(link5, link6, obj_a, obj_b))  < 0.20 + em) n += 1.0;  // gap: forearm->wrist
  if (std::sqrt(SegmentDistSq(link6, link7, obj_a, obj_b))  < 0.18 + em) n += 1.0;  // gap: link6->wrist
  if (std::sqrt(SegmentDistSq(link7, linke7, obj_a, obj_b)) < 0.18 + em) n += 1.0;
  ...
  if (std::sqrt(SegmentDistSq(tip_p, tip_p, obj_a, obj_b)) < tip_thr + em) n += 1.0;
...
int CostJointVelocity(const mjModel* /*model*/, const mjData* data, double* residual) {
  // QUADRATIC (raw qdot) -> norm=0. Kept quadratic so it doesn't dominate the
  // min-max normalization (the kL2 |qdot|+gain*excess form spikes on fast
  // samples and squashes Reach). Tune SPEED via the sensor weight (higher=calmer).
  //
  // MJPC_QDOT_MAX (rad/s; yaml `qdot_max:` publishes it): when set >0, penalize
  // only the EXCESS above the cap (quadratic hinge). Below-cap motion is FREE, so
  // final convergence is unaffected while the initial sprint (which clips the
  // obstacle right in front of the start pose) is blocked. Unset -> original raw
  // quadratic behaviour.
  static const double qmax = []() {
    const char* e = std::getenv("MJPC_QDOT_MAX");
    return (e && e[0]) ? std::atof(e) : -1.0;
  }();
  const double* qdot = data->qvel;
  if (qmax > 0.0) {
    // EXCESS-ONLY hinge (quadratic on (|qdot|-qmax)^+): free below the cap so
    // convergence is untouched; the overall pace is tuned by LOWERING the cap,
    // not by re-adding a base |qdot| term — the "|qdot|+gain*excess" combined
    // form was already tried and rejected (see comment above: it spikes on
    // fast samples and squashes Reach in the min-max normalization; verified
    // again 2026-07-14: the arm ignores the target and drifts into the
    // obstacle).
    for (int i = 0; i < 7; ++i) {
      const double ex = std::fabs(qdot[i]) - qmax;
      residual[i] = ex > 0.0 ? ex : 0.0;
    }
  } else {
    for (int i = 0; i < 7; ++i) residual[i] = qdot[i];
  }
  return 7;
}

int CostJointLimit(const mjModel* model, const mjData* data, double* residual) {
  // Saturation margin: keep joints away from their mechanical limits. Mirrors
  // MPPI_tau.cu, which used limits pulled ~0.1-0.15 rad INSIDE the true range
  // (q_limit_min/max, line 2324) + a hard penalty when exceeded. Here we hinge
  // on the margined range [qmin+m, qmax-m]: residual = 0 while safely inside,
  // then ramps up as a joint enters the margin band / approaches the limit, so
  // MPPI steers away early. margin m via numeric/yaml "joint_limit_margin"
  // (env MJPC_JLIM_MARGIN). Weighted quadratically by the "joint_limit" sensor.
  double m = 0.10;
  if (const char* e = std::getenv("MJPC_JLIM_MARGIN"); e && e[0]) m = std::atof(e);
  else m = GetNumberOrDefault(0.10, model, "joint_limit_margin");
  const double* q = data->qpos;
  for (int i = 0; i < 7; ++i) {
    const double qmin = model->jnt_range[i * 2 + 0] + m;
    const double qmax = model->jnt_range[i * 2 + 1] - m;
    const double lo = std::max(0.0, qmin - q[i]);  // past lower margined limit
    const double hi = std::max(0.0, q[i] - qmax);  // past upper margined limit
    residual[i] = lo + hi;
  }
  return 7;
}
...
int CostFMTrack(const mjModel* model, const mjData* data, double* residual) {
  static double scale = []() {
    // Pure warm-start (MJPC_FM_MODE=wta) carries NO proximal cost: alpha
    // (FM_TRACK_SCALE) acts in cost mode only, so FM_MODE and alpha are
    // orthogonal knobs. (Mode is fixed at launch, so caching in the static is safe.)
    if (const char* m = std::getenv("MJPC_FM_MODE"); m && std::strcmp(m, "wta") == 0) {
      return 0.0;
    }
