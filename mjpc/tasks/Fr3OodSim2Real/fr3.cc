...
#include "mjpc/task.h"
#include "mjpc/tasks/Fr3OodSim2Real/cost_fn.h"
#include "mjpc/utilities.h"

namespace mjpc {

std::string FR3OodSim2Real::XmlPath() const {
  return GetModelPath("Fr3OodSim2Real/task.xml");
}
std::string FR3OodSim2Real::Name() const { return "FR3_OOD_Sim2Real"; }

void FR3OodSim2Real::ResidualFn::Residual(const mjModel* model, const mjData* data,
                                    double* residual) const {
  int counter = 0;
  counter += fr3ood::CostPosition(model, data, residual + counter);
  counter += fr3ood::CostOrientation(model, data, residual + counter);
  counter += fr3ood::CostJointCentralize(model, data, residual + counter);
  counter += fr3ood::CostJointVelocity(model, data, residual + counter);
  counter += fr3ood::CostControl(model, data, residual + counter);
  counter += fr3ood::CostFMTrack(model, data, residual + counter);
...
void FR3OodSim2Real::TransitionLocked(mjModel* model, mjData* data) {
