#!/usr/bin/env python3
"""Compare FlowMPPI TE on vs off."""
import csv, glob, math, statistics as st

ROOT = "out/sweep_flowmppi_te"
WARMUP = 5.0
F_THRESH = 1.0
...
print(f"{'label':10s}  {'contact %':14s}  {'Fz mean':14s}  {'Fz p95':14s}  {'xy [mm]':12s}")
print("-"*80)
for lbl in ["te_off", "te_on"]:
    files = sorted(glob.glob(f"{ROOT}/force_{lbl}_s*.csv"))
    r = agg(files)
    if not r: continue
    cm,cs,fm,fs,pm,ps,xm,xs = r
    print(f"{lbl:10s}  {cm:5.1f}±{cs:4.1f}    {fm:+6.2f}±{fs:5.2f}   {pm:+6.2f}±{ps:5.2f}   {xm:5.2f}±{xs:4.2f}")
